...
SEED = 2026
N_SEEDS = 20 # Tăng lên 20 seed (Titan mode)
...
def get_models(seed: int, is_aov: bool = False):
    ...
    # CHÌA KHÓA: Thay đổi toàn bộ sang MAE Objective (reg:absoluteerror)
    return [
...
def main():
    print("🚀 [V29 MAE TITAN COMPONENTS] Khởi động...")
    print("🔑 Chìa khóa: Tối ưu hóa trực tiếp hàm mục tiêu MAE thay vì Tweedie.")
    ...
    out_path = OUTPUT_DIR / "submission_v29_mae_comp_raw.csv"
    ...
    if ref_path.exists():
        ...
        for alpha in [0.20, 0.30, 0.40]:
            ...
            a_path = OUTPUT_DIR / f"submission_v29_mae_comp_anchor_{int(alpha*100)}.csv"
            anchored.to_csv(a_path, index=False)
            print(f"✅ Đã lưu Anchor {int(alpha*100)}%: {a_path.name}")
